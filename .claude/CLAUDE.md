...
## Progetto
- Nome: agenda google pwa
- Codice: 5lfpOdDC
- Tech stack: react-vite-ts
- Repository: mrshu1978/axiom-5lfpOdDC
...
- **Python**: `pip install -r requirements.txt` · crea venv se manca: `python -m venv .venv && source .venv/bin/activate` · per Django/Flask: non tentare `manage.py migrate` senza DB attivo
- **.NET**: `dotnet restore` poi `dotnet build` · per EF Core: crea le migration ma NON eseguire `dotnet ef database update`
- **Go**: `go mod tidy` · per workspace: `go work sync`
- **Java**: `mvn clean install -DskipTests` o `gradle build -x test` · NON eseguire test (non c'è DB/servizi esterni)
- **PHP**: `composer install --no-interaction --no-scripts`
- **Rust**: `cargo build` · per workspace: `cargo build --workspace`
...
- Git push: `git push origin HEAD`. Se fallisce: `git pull --rebase origin HEAD` poi ripush
- **MAI abbandonare senza commit+push**: anche se incompleto, committa ciò che hai fatto
